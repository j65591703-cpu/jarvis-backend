...
app.post('/ask', async (req, res) => {
  try {
    ...
    const langLine = (language && language !== 'हिन्दी' && language !== 'Hindi')
      ? `IMPORTANT: Respond strictly and only in "${language}" (in that language's native script), even if the question was asked in Hindi or English.`
      : 'IMPORTANT: Respond strictly and only in natural, conversational Hindi (Devanagari script), in the way a native Hindi speaker would actually talk — not textbook/formal Hindi, not Hinglish/Roman script, and do not switch to English or any other language unless the user explicitly asks you to switch languages.';
